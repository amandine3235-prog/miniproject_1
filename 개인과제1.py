import pickle #pickle(객체의 형태를 그대로 유지하면서 파일에 저장하고 불러올 수 있게 하는 모듈)을 불러온다 -273페이지
import os #os(환경 변수나 디렉터리, 파일 등의 OS 자원을 제어할 수 있게 해주는 모듈)을 불러온다
from datetime import datetime #datetime() 모듈을 불러온다

DATA_FILE = "todo_data.pkl" #.pkl 파일 : pickle 파일 

class User: #User 클래스 선언
    def __init__(self, username, password, role="user"): #User 클래스 변수 초기화
        self.username = username #user의 id 생성자
        self.password = password #user의 password 생성자
        self.role = role  # user or admin


class Task: #Task 클래스 선언 
    def __init__(self, title, owner, priority): #Task 클래스 변수 초기화
        self.title = title
        self.owner = owner
